...
@RestController
@RequestMapping("/rag")
@Tag(name = "RAG Vector Database", description = "Endpoints for managing RAG (Retrieval-Augmented Generation) vector database operations")
@OpenAPIDefinition(
    info = @Info(
        title = "RAG Vector Database API",
        version = "1.0",
        description = "APIs for storing and retrieving data from vector database for RAG workflows"
    )
)
public class RagVectorDataController {
  ...
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "Data successfully stored in vector database"),
      @ApiResponse(responseCode = "400", description = "Invalid input data"),
      @ApiResponse(responseCode = "500", description = "Internal server error")
  })
  public void storeDataIntoVectorDatabase(
      @RequestBody List<String> companyDataList) {
  ...
  @Operation(
      ...
      description = "Performs a similarity search on the vector database and returns relevant documents along with AI-generated response"
  )
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = "200",
